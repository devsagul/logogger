...
func (reporter *HTTPReporter) ReportMetrics(ctx context.Context, l []schema.Metrics, host string) error {
	reporter.wg.Add(1)
...
func (reporter *HTTPReporter) ReportMetricsBatches(ctx context.Context, l []schema.Metrics, host string) error {
	reporter.wg.Add(1)
	defer reporter.wg.Done()
...
func (reporter *HTTPReporter) Shutdown() {
	reporter.wg.Wait()
}

func NewHTTPReporter(encryptor crypt.Encryptor) (*HTTPReporter, error) {
	ip, err := getRealIP()
	if err != nil {
		return nil, err
	}
	return &HTTPReporter{batches: true, encryptor: encryptor, wg: sync.WaitGroup{}, ip: ip}, nil
}
